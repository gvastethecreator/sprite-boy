...
export class CanvasRenderer {
      ...
      for (let c = 0; c < cols; c++) {
        ...
        if (slot && assetCache[slot.assetId] && draggingSlotIndex !== idx) {
          ...
          let dw = img.width,
            dh = img.height;
          if (slot.fitMode === "fit") {
            const rat = Math.min(cellW / dw, cellH / dh);
            dw *= rat;
            dh *= rat;
          } else if (slot.fitMode === "fill") {
            const rat = Math.max(cellW / dw, cellH / dh);
            dw *= rat;
            dh *= rat;
          ...
          const align = this.getAlignmentCoords(
          ...
          ctx.translate(
            x + align.x + scaledW / 2 + slot.offsetX,
            y + align.y + scaledH / 2 + slot.offsetY,
          );
  ...
  private static renderSlicerMode(state: Partial<RenderContext>) {
    ...
    if (frames.length === 0) ctx.drawImage(slicerImgObj, 0, 0);
      ...
      frames.forEach((f) => {
        if (!f.hidden) ctx.drawImage(slicerImgObj, f.x, f.y, f.w, f.h, f.x, f.y, f.w, f.h);
      });
    }
  ...
  private static renderDualView(state: RenderContext, cw: number, ch: number) {
    ...
    if (currentKf) {
      ...
      if (slot && assetCache[slot.assetId]) {
        ...
        this.drawPivotMarker(
          ctx,
          img.width,
          img.height,
          currentKf.pivotX,
          currentKf.pivotY,
          sBase * Math.max(sx, sy),
        );
        ctx.restore();
