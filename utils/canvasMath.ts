...
export const getResizeHandle = (mx: number, my: number, x: number, y: number, w: number, h: number, scale: number): ResizeHandle | null => {
    ...
    const within = (px: number, py: number) => 
        mx >= px - half - padding && mx <= px + s + padding && 
        my >= py - half - padding && my <= py + s + padding;
...
export const calculateSnapping = (
    ...
    for (const tx of targetsX) {
        if (Math.abs(myLeft - tx) < threshold) { newX = tx; guides.push({type: 'vertical', position: tx, start: Math.min(y, 0), end: Math.max(y+h, canvasH)}); break; }
        if (Math.abs(myRight - tx) < threshold) { newX = tx - w; guides.push({type: 'vertical', position: tx, start: Math.min(y, 0), end: Math.max(y+h, canvasH)}); break; }
        if (Math.abs(myCenterX - tx) < threshold) { newX = tx - w/2; guides.push({type: 'vertical', position: tx, start: Math.min(y, 0), end: Math.max(y+h, canvasH)}); break; }
    }

    // Check Vertical Snaps (Horizontal Guides)
    for (const ty of targetsY) {
        if (Math.abs(myTop - ty) < threshold) { newY = ty; guides.push({type: 'horizontal', position: ty, start: Math.min(x, 0), end: Math.max(x+w, canvasW)}); break; }
        if (Math.abs(myBottom - ty) < threshold) { newY = ty - h; guides.push({type: 'horizontal', position: ty, start: Math.min(x, 0), end: Math.max(x+w, canvasW)}); break; }
        if (Math.abs(myCenterY - ty) < threshold) { newY = ty - h/2; guides.push({type: 'horizontal', position: ty, start: Math.min(x, 0), end: Math.max(x+w, canvasW)}); break; }
    }
