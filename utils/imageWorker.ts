export function detectSprites(data: Uint8ClampedArray, width: number, height: number, threshold: number) {
...
export function detectSpriteAt(data: Uint8ClampedArray, width: number, height: number, startX: number, startY: number, threshold: number) {
    const getIdx = (x: number, y: number) => (y * width + x) * 4;
...
export function removeBackground(data: Uint8ClampedArray, width: number, height: number, targetHex: string, tolerance: number, softness: number) {
...
self.onmessage = async (e) => {
    ...
    try {
        ...
        switch (type) {
            case 'DETECT_SPRITES':
                result = detectSprites(new Uint8ClampedArray(payload.buffer), payload.width, payload.height, payload.threshold);
                break;
                
            case 'REMOVE_BG':
                const data = new Uint8ClampedArray(payload.buffer);
                removeBackground(data, payload.width, payload.height, payload.targetHex, payload.tolerance, payload.softness);
                result = { width: payload.width, height: payload.height, buffer: data.buffer };
                break;
                
            case 'DETECT_ONE':
                result = detectSpriteAt(new Uint8ClampedArray(payload.buffer), payload.width, payload.height, payload.startX, payload.startY, payload.threshold);
                break;
