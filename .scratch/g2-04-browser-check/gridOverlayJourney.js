...
// tests/browser/gridOverlayJourney.tsx
var import_react2 = __toESM(require_react(), 1);
var import_client = __toESM(require_client(), 1);

// core/project/schema.ts
var WORKSPACE_IDS = Object.freeze([
  "assets",
  "slice",
  "compose",
  "animate",
...
var PROJECT_RECORD_COLLECTIONS = [
  "assets",
  "regions",
  "layers",
  "compositions",
...
// core/processing/gridProcessingLimits.ts
var GRID_PROCESSING_LIMITS = Object.freeze({
  maxIdentifierLength: 256,
  maxDimension: 16384,
  maxSourcePixels: 67108864,
  maxResultCount: 4096,
  maxResultPixels: 67108864,
  maxProgressTotal: 67108864,
  maxPixelSize: 4096,
  maxPaletteColors: 256,
  maxWarnings: 16
});

// core/project/applyCompositionCommands.ts
var MAX_COMPOSITION_PIXELS = 64 * 1024 * 1024;

// core/project/impact.ts
var COLLECTION_RANK = new Map(PROJECT_RECORD_COLLECTIONS.map((collection, index) => [collection, index]));

// core/project/applyDestructiveCommands.ts
var PURE_REMOVE_TYPES = new Set([
  "asset.remove",
  "region.remove",
...
// core/processing/gridLayoutValidation.ts
var EXPECTED_SOURCE_KEYS = Object.freeze(["width", "height"]);
var EXPECTED_LAYOUT_KEYS = Object.freeze(["mode", "manual"]);
var EXPECTED_MANUAL_KEYS = Object.freeze(["rows", "cols"]);
// core/processing/gridProcessingProtocol.ts
var GRID_PROCESSING_STAGES = Object.freeze([
  "decode",
  "detect",
  "chroma",
  "crop",
...
var GRID_PROCESSING_OPERATIONS = Object.freeze([
  "chroma",
  "crop",
  "resize",
  "quantize"
]);
var GRID_PROCESSING_WARNING_CODES = Object.freeze([
  "grid-detection-fallback",
  "empty-output",
  "pixel-size-clamped",
  "palette-reduced"
]);
var GRID_PROCESSING_ERROR_CODES = Object.freeze([
  "invalid-input",
  "decode",
  "detect",
  "memory",
...
var ARRAY_BUFFER_BYTE_LENGTH_GETTER = Object.getOwnPropertyDescriptor(ArrayBuffer.prototype, "byteLength")?.get;
var ARRAY_BUFFER_RESIZABLE_GETTER = Object.getOwnPropertyDescriptor(ArrayBuffer.prototype, "resizable")?.get;
var ARRAY_BUFFER_MAX_BYTE_LENGTH_GETTER = Object.getOwnPropertyDescriptor(ArrayBuffer.prototype, "maxByteLength")?.get;

// core/processing/gridProcessingGeometry.ts
function requireInteger(value, minimum, maximum, label) {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || !Number.isFinite(value) || Object.is(value, -0) || value < minimum || value > maximum) {
    throw new TypeError(`${label} must be a canonical safe integer from ${minimum} to ${maximum}.`);
  }
  return value;
}
function requireSourceDimensions(width, height) {
...
function buildManualGrid(totalWidth, totalHeight, rows, cols) {
  ...
  if (safeRows > dimensions.height || safeCols > dimensions.width || safeRows * safeCols > GRID_PROCESSING_LIMITS.maxResultCount) {
...
// features/slice/grid/SliceGridOverlay.tsx
var import_react = __toESM(require_react(), 1);

// features/slice/grid/gridOverlayGeometry.ts
var MAX_CSS_SURFACE = 65536;
var MAX_DEVICE_PIXEL_RATIO = 8;
var MAX_SCALE = 4096;
var MAX_ABSOLUTE_OFFSET = 1e9;
function requireCanonicalInteger(value, minimum, maximum, label) {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || !Number.isFinite(value) || Object.is(value, -0) || value < minimum || value > maximum) {
    throw new TypeError(`${label} is not a canonical grid overlay integer.`);
  }
  return value;
}
function requireCanonicalNumber(value, minimum, maximum, label) {
...
function exactOwnDataRecord(record, keys) {
  if (record === null || typeof record !== "object" || Array.isArray(record)) {
    throw new TypeError("Grid overlay input must be an object.");
  }
  let ownKeys;
...
function readCells(value, sourceWidth, sourceHeight) {
  if (!Array.isArray(value))
    throw new TypeError("Grid overlay cells must be an array.");
  const length = Object.getOwnPropertyDescriptor(value, "length");
  if (!length || !("value" in length) || !Number.isSafeInteger(length.value) || length.value < 1 || length.value > GRID_PROCESSING_LIMITS.maxResultCount || Reflect.ownKeys(value).length !== length.value + 1) {
...
function projectRect(rect, scale, offsetX, offsetY) {
...
function cssRectToDevice(rect, dpr) {
...
function projectGridOverlay(cellsValue, sourceWidthValue, sourceHeightValue, transformValue, surfaceValue) {
  const sourceWidth = requireCanonicalInteger(sourceWidthValue, 1, GRID_PROCESSING_LIMITS.maxDimension, "sourceWidth");
  const sourceHeight = requireCanonicalInteger(sourceHeightValue, 1, GRID_PROCESSING_LIMITS.maxDimension, "sourceHeight");
  ...
  const offsetX = requireCanonicalNumber(readOwnData(offsetValue, "x"), -MAX_ABSOLUTE_OFFSET, MAX_ABSOLUTE_OFFSET, "offset.x");
  const offsetY = requireCanonicalNumber(readOwnData(offsetValue, "y"), -MAX_ABSOLUTE_OFFSET, MAX_ABSOLUTE_OFFSET, "offset.y");
  ...
  const dpr = requireCanonicalNumber(readOwnData(surfaceValue, "devicePixelRatio"), 0.25, MAX_DEVICE_PIXEL_RATIO, "surface.devicePixelRatio");
  const backingWidth = Math.round(surfaceWidth * dpr);
  const backingHeight = Math.round(surfaceHeight * dpr);
  if (backingWidth > MAX_CSS_SURFACE * MAX_DEVICE_PIXEL_RATIO || backingHeight > MAX_CSS_SURFACE * MAX_DEVICE_PIXEL_RATIO) {
  ...
  const projectedCells = cells.map((source, index) => {
...
var DEFAULT_GRID_OVERLAY_PAINT_STYLE = Object.freeze({
  lineColor: "rgba(125, 211, 252, 0.92)",
  fillColor: "rgba(14, 165, 233, 0.055)",
  outerLineColor: "rgba(224, 242, 254, 0.98)",
  lineWidthCss: 1
});
function paintGridOverlay(context, projection, style = DEFAULT_GRID_OVERLAY_PAINT_STYLE) {
  ...
  context.strokeRect(sourceBoundsCss.x, sourceBoundsCss.y, sourceBoundsCss.width, sourceBoundsCss.height);
  context.setTransform(1, 0, 0, 1, 0, 0);
}

// features/slice/grid/SliceGridOverlay.tsx
var jsx_dev_runtime = __toESM(require_jsx_dev_runtime(), 1);
function currentDpr(override) {
...
function disconnectNoThrow(observer) {
  try {
    observer?.disconnect();
  } catch {}
}
function removeListenerNoThrow(target, listener) {
  try {
    target?.removeEventListener("resize", listener);
  } catch {}
}
var SliceGridOverlay = ({
  sourceDimensions,
  effectiveLayout,
  transform,
  paintStyle,
  ...
  if (sourceDimensions === null || effectiveLayout === null)
    return null;
  return /* @__PURE__ */ jsx_dev_runtime.jsxDEV(SliceGridOverlayCanvas, {
    sourceDimensions,
    effectiveLayout,
    transform,
    paintStyle,
...
var SliceGridOverlayCanvas = ({
  ...
  const hostRef = import_react.useRef(null);
  const canvasRef = import_react.useRef(null);
  const drawCountRef = import_react.useRef(0);
  const [surface, setSurface] = import_react.useState(() => Object.freeze({
    width: 0,
    height: 0,
  ...
  import_react.useLayoutEffect(() => {
    ...
    if (!canvas)
      return;
    const projection = projectGridOverlay(effectiveLayout.cells, sourceDimensions.width, sourceDimensions.height, transform, {
      width: surface.width,
...
var SOURCE_WIDTH = 7;
var SOURCE_HEIGHT = 5;
var CELLS = buildManualGrid(SOURCE_WIDTH, SOURCE_HEIGHT, 2, 3);
...
function requireCanvas() {
  ...
  if (!canvas)
    throw new Error("G2-04 overlay canvas is unavailable.");
  return canvas;
}
function Harness() {
  const [transform, setTransform] = import_react2.useState(() => Object.freeze({
    scale: 36,
    offset: Object.freeze({ x: 90, y: 70 })
  }));
  const [stageSize, setStageSize] = import_react2.useState(() => ({ width: 640, height: 360 }));
  const [displayDpr, setDisplayDpr] = import_react2.useState(() => window.devicePixelRatio);
  const sourceRef = import_react2.useRef(null);
  const stageRef = import_react2.useRef(null);
  const pointerTargetRef = import_react2.useRef("none");
  ...
  import_react2.useLayoutEffect(() => {
    ...
    if (!source || !context)
      throw new Error("G2-04 source canvas is unavailable.");
  ...
  import_react2.useLayoutEffect(() => {
    window.__gridOverlayHarness = {
      ...
      snapshot: () => {
        ...
        if (!stage)
          throw new Error("G2-04 stage is unavailable.");
        const context = canvas.getContext("2d");
        if (!context)
          throw new Error("G2-04 overlay context is unavailable.");
        ...
        const alpha = canvas.width > 0 && canvas.height > 0 ? context.getImageData(sampleX, sampleY, 1, 1).data[3] ?? 0 : 0;
        return {
  ...
  return /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("main", {
    className: "shell",
    children: [
          ...
          /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("div", {
            children: [
              /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("h1", {
                children: "Slice grid overlay"
              }, undefined, false, undefined, this),
              /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("p", {
                children: "Canonical source geometry · CSS viewport · DPR backing"
              }, undefined, false, undefined, this)
            ]
      ...
      /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("div", {
        ...
        children: [
          /* @__PURE__ */ jsx_dev_runtime2.jsxDEV("div", {
            ...
            onPointerDown: (event) => {
              pointerTargetRef.current = event.target.dataset.gridOverlayStage !== undefined ? "stage" : "source";
            },
            children: [
              ...
              /* @__PURE__ */ jsx_dev_runtime2.jsxDEV(SliceGridOverlay, {
...
if (!rootElement)
  throw new Error("G2-04 harness root is unavailable.");
var root = import_client.createRoot(rootElement);
root.render(/* @__PURE__ */ jsx_dev_runtime2.jsxDEV(Harness, {}, undefined, false, undefined, this));
