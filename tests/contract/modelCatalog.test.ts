...
describe("local model catalog (M1-01)", () => {
  it("pins the two supported profiles to immutable Hub revisions and hashes", () => {
    expect(MODEL_CATALOG_SCHEMA_VERSION).toBe(1);
    expect(Object.keys(LOCAL_MODEL_CATALOG)).toEqual(["birefnet-lite-512", "rmbg-2.0"]);
    ...
    for (const model of Object.values(LOCAL_MODEL_CATALOG)) {
      ...
      for (const file of model.files) {
        ...
        expect(file.digest.value).toMatch(file.digest.algorithm === "sha256"
          ? /^[0-9a-f]{64}$/u
          : /^[0-9a-f]{40}$/u);
        expect(file.downloadUrl).toContain(`/resolve/${model.revision}/${file.path}`);
      }
    }
  });
