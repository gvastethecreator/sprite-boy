...
function coverageSummary() {
  return {
    total: {
      statements: { total: 9170, covered: 8254, skipped: 0, pct: 90.01 },
      branches: { total: 6753, covered: 5813, skipped: 0, pct: 86.08 },
      functions: { total: 1317, covered: 1249, skipped: 0, pct: 94.83 },
      lines: { total: 8146, covered: 7548, skipped: 0, pct: 92.65 },
    },
...
describe("canonical coverage policy", () => {
  ...
  it("passes both the raised ratchet and release target after measured improvement", () => {
    ...
    expect(release).toMatchObject({ status: "pass", includesCoreProject: true, below: [] });
  });
...
describe("initial bundle policy", () => {
  it("extracts only local initial assets and keeps the current ratchet separate from release", () => {
