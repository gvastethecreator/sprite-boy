...
const ToastItem: React.FC<{ toast: ToastData; onRemove: (id: string) => void }> = ({ toast, onRemove }) => {
    ...
    return (
        <div
            ...
            `}
        >
            <div ref={barRef} className={`absolute bottom-0 left-0 h-[2px] bg-current opacity-80 w-full origin-left ${toast.type === 'error' ? 'text-red-500' :
                    toast.type === 'info' ? 'text-blue-500' :
                        'text-green-500'
                }`}></div>

            <div className="flex items-stretch w-full p-3 gap-3 relative z-10">
                <div className={`shrink-0 flex items-center justify-center rounded-full w-6 h-6 ${toast.type === 'error' ? 'text-red-500 bg-red-500/10' :
