...
const Timeline: React.FC = () => {
  const {
    ...
    animations,
    activeAnimationId,
    handleAddKeyframe: onAddKeyframe,
    handleReorderFrames: onReorderFrames,
    playbackFrameIndex,
    setPlaybackFrameIndex,
    isPlaying,
    setIsPlaying,
    handleStepFrame: onStepFrame,
    handleAddKeyframeFromAsset: onAddKeyframeFromAsset,
  } = useProject();
  ...
  const handleContainerDragOver = (e: React.DragEvent) => {
    ...
    const isValid =
      types.includes(DND_ASSET_TYPE.toLowerCase()) ||
      ...
      return (
        <div className="w-full h-full flex items-center justify-center p-0.5">
          <img
            ...
            style={{
              transform: `scaleX(${source.flipX ? -1 : 1}) scaleY(${source.flipY ? -1 : 1})`,
            }}
            alt=""
          />
        </div>
      ...
      <div className="h-10 bg-surface border-b border-white/5 flex items-center justify-between px-4 shrink-0 select-none">
        <div className="flex items-center gap-3 text-textMuted">
          ...
          {isAnimationMode && activeAnimation && (
            <span className="text-textMain font-mono text-sm px-2">
              {(playbackFrameIndex! + 1).toString().padStart(2, "0")}
              <span className="opacity-50">/</span>
              {activeAnimation.keyframes.length.toString().padStart(2, "0")}
            </span>
          )}
        </div>
        ...
        {isAnimationMode && (
          <div className="flex items-center gap-2 bg-input p-1 rounded-md border border-white/5">
            <button
              ...
              className="p-1.5 hover:bg-white/10 rounded-sm text-textMuted hover:text-white transition-all active:scale-90"
            >
              <SkipBack size={12} />
            </button>
            <button
              onClick={() => setIsPlaying?.(!isPlaying)}
              className={`w-6 h-6 rounded-sm flex items-center justify-center transition-all ${isPlaying ? "bg-accent text-white" : "text-textMain hover:bg-white/10"}`}
            >
              {isPlaying ? (
        ...
        <div className="flex gap-2 h-full min-w-full items-center px-4 z-10 pb-4">
          {isAnimationMode && activeAnimation ? (
            activeAnimation.keyframes.length === 0 ? (
              <div
                className="w-full h-32 flex flex-col items-center justify-center border border-dashed border-white/10 rounded-lg bg-white/5 hover:bg-white/10 hover:border-accent/50 group cursor-pointer transition-all select-none"
                onClick={() => setShowTray(true)}
              >
                <div className="p-4 rounded-full bg-surface mb-3">
                  <Layers
                    size={24}
                ...
                <div className="flex items-center gap-1 text-xs text-textMuted mt-1">
                  <span>Drag frames/assets here or click "Add Frames"</span>
                </div>
              </div>
            ) : (
              activeAnimation.keyframes.map((kf, idx) => {
                const source =
                  slicerSources.find((s) => s.id === kf.sourceIndex) ||
                  builderSources.find((s) => s.id === kf.sourceIndex);
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute -top-3 left-1/2 -translate-x-1/2 transition-opacity duration-200 ${isPlayingFrame ? "opacity-100" : "opacity-0"}`}
                    >
                    ...
                    <div
                      className={`flex-1 rounded-lg flex flex-col overflow-hidden transition-all duration-200 border ${isPlayingFrame ? "bg-surface border-accent ring-1 ring-accent" : isSelected ? "bg-panel border-white/40" : "bg-panel border-white/10 hover:border-white/30"}`}
                    >
                      <div className="flex-1 relative p-2 bg-checkered">
                        {source ? (
                          renderSourceThumbnail(source)
                        ) : (
                      ...
                      <div
                        className={`h-6 text-[10px] font-mono flex items-center justify-center border-t border-white/5 font-bold ${isPlayingFrame ? "bg-accent text-white" : "bg-surface text-textMuted"}`}
                      >
                        {idx + 1}
                      </div>
                    </div>
            ...
            slicerSources.map((source) => (
              <div
                key={source.id}
                onClick={() => onSelectFrame(source.id)}
                className="relative group flex-shrink-0 w-[72px] h-[90px] cursor-pointer select-none flex flex-col"
              >
                <div
                  className={`flex-1 rounded-lg flex flex-col overflow-hidden transition-all duration-200 border ${selectedFrameIndex === source.id ? "bg-panel border-accent ring-1 ring-accent shadow-sm" : "bg-panel border-white/10 hover:border-white/30"}`}
                >
                  <div className="flex-1 relative p-1.5 bg-checkered">
                    {renderSourceThumbnail(source)}
                  </div>
                  <div
                    className={`h-5 text-[10px] font-mono flex items-center justify-center border-t border-white/5 ${selectedFrameIndex === source.id ? "bg-accent text-white" : "bg-surface text-textMuted"}`}
                  >
                    {source.id}
                  </div>
                </div>
        ...
        <div className="absolute bottom-full left-2 right-2 h-[210px] bg-panel border border-white/10 shadow-xl z-30 flex flex-col rounded-t-xl animate-slide-up origin-bottom">
          <div className="h-10 bg-surface px-4 flex items-center justify-between border-b border-white/5 rounded-t-xl select-none">
            <div className="flex items-center gap-4">
              <span className="text-xs font-bold text-textMain uppercase tracking-wider flex items-center gap-2">
                <Grid size={14} className="text-accent" /> Frame Picker
              </span>
            ...
            <button
              ...
              className="hover:bg-white/10 p-1.5 rounded-full active:scale-90 transition-transform"
            >
              <X size={14} className="text-textMuted hover:text-white" />
            </button>
          </div>
            ...
            {availableSources.length === 0 ? (
              ...
              availableSources.map((source, i) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 right-0 px-1 bg-black/60 text-[8px] text-white font-mono z-30">
                    #{source.id}
                  </div>
                </div>
              ))
