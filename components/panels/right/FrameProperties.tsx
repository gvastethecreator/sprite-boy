...
const FrameProperties: React.FC<FramePropertiesProps> = ({
  frame,
  onUpdate,
  onCommit,
  onDuplicate,
    ...
    <Section>
      <div className="grid grid-cols-2 gap-4">
        <NumberControl
          label="X"
          value={frame.x}
          onChange={(v) => onUpdate(frame.id, { x: v })}
          onAfterChange={(v) => onCommit(frame.id, { x: v })}
          min={0}
        />
        <NumberControl
          label="Y"
          value={frame.y}
          onChange={(v) => onUpdate(frame.id, { y: v })}
          onAfterChange={(v) => onCommit(frame.id, { y: v })}
          min={0}
        />
        <NumberControl
          label="W"
          value={frame.w}
          onChange={(v) => onUpdate(frame.id, { w: v })}
          onAfterChange={(v) => onCommit(frame.id, { w: v })}
          min={1}
        />
        <NumberControl
          label="H"
          value={frame.h}
          onChange={(v) => onUpdate(frame.id, { h: v })}
          onAfterChange={(v) => onCommit(frame.id, { h: v })}
          min={1}
        />
      </div>
