...
const FrameList: React.FC<FrameListProps> = ({
    frames, imageMeta, selectedIndex, builderSlots, builderAssets,
    onSelectFrame, onDeleteFrame, onToAsset, onToggleVisibility
}) => {
    ...
    const handleDragStart = (e: React.DragEvent, frameId: number) => {
        ...
        dragPreview.className = "w-12 h-12 bg-accent/50 border-2 border-accent rounded-lg fixed -top-40";
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 bg-panel-gradient">
            <div className="h-10 bg-white/5 flex items-center px-4 border-b border-white/5">
                ...
                <span className="text-[10px] font-bold text-textMuted uppercase tracking-wider">Workspace Resources ({frames.length})</span>
            </div>
                            ...
                            >
                                ...
                                <div className="w-12 h-12 bg-checkered rounded-lg border border-white/10 flex items-center justify-center overflow-hidden shrink-0 relative">
                                    {asset ? (
                                        <img
                                            ...
                                            style={{ transform: `scaleX(${slot?.flipX ? -1 : 1}) scaleY(${slot?.flipY ? -1 : 1})` }}
                                            alt=""
                                        />
                                    ) : imageMeta ? (
                                        <div
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="text-[11px] font-bold text-textMain truncate">
                                        Frame #{frame.id} {asset && <span className="text-[9px] text-accent font-normal">(Occupied)</span>}
                                    </div>
                                ...
                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button
                                        onClick={(e) => { e.stopPropagation(); onToggleVisibility?.(frame.id); }}
