...
import { Sparkles, BrainCircuit, Target, Plus, ArrowRight, Maximize, X, Wand2, Paperclip } from 'lucide-react';
import { GenerationPanelState, DND_ASSET_TYPE, DND_KEYFRAME_TYPE, DND_FRAME_TYPE, AIModelId, AIGenerationMode } from '../../../types';
...
const GenerationPanel: React.FC<GenerationPanelProps> = ({ state, setState, onDrop, onClear, onRun }) => {
        ...
        <div className="flex flex-col h-full bg-panel">
            ...
            <div className="p-4 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                <p className="text-[10px] text-textMuted leading-relaxed">
                    Generate new sprites and assets. Results will be added to your <strong>Global Library</strong> automatically.
                </p>

                {/* Model Selector */}
                <div className="space-y-2">
                    <label className="text-[10px] font-bold text-textMuted uppercase flex items-center gap-2 tracking-wider"><BrainCircuit size={12} /> AI Engine</label>
                    <select
                        value={state.model}
                ...
                {/* Mode Selector */}
                <div className="space-y-2">
                    <label className="text-[10px] font-bold text-textMuted uppercase flex items-center gap-2 tracking-wider"><Target size={12} /> Output Mode</label>
                    <select
                        value={state.mode}
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        <label className="text-[10px] font-bold text-textMuted uppercase tracking-wider flex items-center gap-2"><Paperclip size={12} /> Attachments</label>
                            ...
                            >
                                {state.contextSlots[i] ? (
                                    <>
                                        ...
                                        <img src={state.contextSlots[i]!.previewSrc} className="w-full h-full object-contain relative z-10 p-1" alt="context" />
                                        <div className="absolute inset-0 z-20 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                                            <button onClick={() => onClear(i)} className="p-1.5 bg-red-500 rounded-full text-white shadow-sm"><X size={12} /></button>
                                        </div>
                                    </>
                                ) : (
                ...
                <div className="space-y-2 pt-2 border-t border-white/5">
                    <label className="text-[10px] font-bold text-textMuted uppercase tracking-wider">Prompt / Instructions</label>
                    <textarea
                        value={state.prompt}
                        ...
                        placeholder={
                            state.mode === 'inbetween' ? "Describe the movement between attachments..." :
                                state.mode === 'new_image' ? "Describe the asset to create (e.g., 'Pixel art sci-fi drone')..." :
            ...
            <div className="p-3 border-t border-white/5 text-center bg-black/20">
                <span className="text-[9px] text-textMuted font-mono opacity-50 uppercase tracking-widest">Local-First Neural Engine</span>
            </div>
        </div>
    );
};
