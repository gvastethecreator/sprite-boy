      ...
      {props.selectedFrame ? (
        <>
          <div className="p-3 rounded bg-surface/30 border border-border/50 text-center">
            <div className="text-[11px] font-bold text-textMain mb-1">
              Frame #{props.selectedFrame.id}
            </div>
            <div className="text-[10px] text-textMuted">Edit Hitboxes</div>
          </div>
          <button
            onClick={() => props.onAddHitbox?.(props.selectedFrame!.id)}
            className="w-full py-2 bg-accent hover:bg-accentHover text-white rounded text-xs font-semibold btn-tactile flex items-center justify-center gap-2"
          >
            <Plus size={14} /> Add Hitbox
          </button>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => props.onCopyHitboxes?.(props.selectedFrame!.id)}
              className="py-1.5 bg-surface hover:bg-tool border border-border/50 rounded text-[10px] text-textMain btn-tactile flex items-center justify-center gap-1"
            >
              <ClipboardCopy size={12} /> Copy
            </button>
            <button
              onClick={() => props.onPasteHitboxes?.(props.selectedFrame!.id)}
              className="py-1.5 bg-surface hover:bg-tool border border-border/50 rounded text-[10px] text-textMain btn-tactile flex items-center justify-center gap-1"
            >
          ...
          <button
            onClick={() => props.onFlipHitboxes?.(props.selectedFrame!.id)}
            className="w-full py-1.5 bg-surface hover:bg-tool border border-border/50 rounded text-[10px] text-textMain btn-tactile"
          >
            Flip Horizontal
          </button>
        </>
