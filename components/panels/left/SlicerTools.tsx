            ...
            <div className="p-4 space-y-5">
                {!isBuilder && (
                    <div className="space-y-3">
                        ...
                        <div className={`border rounded-xl transition-all duration-300 overflow-hidden shadow-inner-depth ${props.isMagicWandActive ? 'border-accent bg-accent/10' : 'border-white/5 bg-black/20'}`}>
                            <button
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <Sparkles size={14} className={props.isMagicWandActive ? 'text-accent drop-shadow-md' : 'text-textMuted'} />
                                    Magic Wand
                                </div>
                                <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${props.isMagicWandActive ? 'bg-accent border-accent' : 'border-white/20 bg-black/40'}`}>
                            ...
                            {props.isMagicWandActive && props.setWandTolerance && (
                                <div className="px-3 pb-3 pt-1 animate-fade-in border-t border-white/5 bg-black/20">
                                    <NumberControl label="Tolerance" value={props.wandTolerance || 30} onChange={props.setWandTolerance} min={1} max={100} slider labelClassName="w-16 text-[9px]" />
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {props.gridConfig && (
                    <div className="border border-white/5 rounded-xl bg-black/20 overflow-hidden shadow-inner-depth animate-slide-up">
                        <button onClick={() => setIsGridOpen(!isGridOpen)} className="w-full flex items-center justify-between p-3 bg-white/5 hover:bg-white/10 transition-colors">
                            ...
                            {isGridOpen ? <ChevronDown size={14} className="text-textMuted" /> : <ChevronRight size={14} className="text-textMuted" />}
                        </button>
                        {isGridOpen && (
                            <div className="p-4 space-y-6 animate-scale-in bg-black/20 shadow-inner-depth">
                                <div className="grid grid-cols-2 gap-x-3 gap-y-6">
                                    <NumberControl label="Rows" value={props.gridConfig.rows} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, rows: Math.max(1, v) })} min={1} max={64} slider labelClassName="w-10 text-[9px]" />
                                    <NumberControl label="Cols" value={props.gridConfig.cols} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, cols: Math.max(1, v) })} min={1} max={64} slider labelClassName="w-10 text-[9px]" />
                                </div>

                                <div className="space-y-6 pt-4 border-t border-white/5">
                                    <div className="grid grid-cols-2 gap-x-3 gap-y-6">
                                        <NumberControl label="Margin X" value={props.gridConfig.marginX} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, marginX: v })} min={0} max={200} slider labelClassName="w-14 text-[9px]" />
                                        <NumberControl label="Margin Y" value={props.gridConfig.marginY} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, marginY: v })} min={0} max={200} slider labelClassName="w-14 text-[9px]" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-x-3 gap-y-6">
                                        <NumberControl label="Cell Gap X" value={props.gridConfig.paddingX} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, paddingX: v })} min={0} max={100} slider labelClassName="w-14 text-[9px]" />
                                        <NumberControl label="Cell Gap Y" value={props.gridConfig.paddingY} onChange={(v) => props.setGridConfig?.({ ...props.gridConfig!, paddingY: v })} min={0} max={100} slider labelClassName="w-14 text-[9px]" />
                                    </div>
                                </div>
