    ...
    <SectionHeader
      ...
      action={
        <button
          ...
          className="p-2 hover:bg-tool rounded text-textMuted hover:text-textMain transition-colors"
        >
          <Plus size={18} />
        </button>
      }
        ...
        animations.map((anim) => (
          ...
          >
            <div className="flex items-center gap-3 truncate min-w-0">
              <div
                className={`p-1.5 rounded-md ${activeAnimationId === anim.id ? "bg-accent text-white" : "bg-surface text-textMuted group-hover:text-textMain"}`}
              >
                <Film size={14} />
              </div>
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onDuplicateAnimation?.(anim.id);
                }}
                className="hover:text-textMain hover:bg-white/10 p-1.5 rounded"
              >
                <Copy size={14} />
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onDeleteAnimation?.(anim.id);
                }}
                className="hover:text-red-400 hover:bg-red-500/10 p-1.5 rounded"
              >
                <Trash2 size={14} />
              </button>
            </div>
