...
const CanvasStatusBar: React.FC<CanvasStatusBarProps> = ({ 
    dragMode, mousePos, viewport, setViewport, onResetView, modifiers, isHoveringInteractive 
}) => {

    const getStatusMessage = () => {
        if (dragMode === DragMode.PAN) return { icon: Hand, text: "Panning view..." };
        if (dragMode === DragMode.SWAP_SLOTS) return { icon: RefreshCcw, text: "Release to swap slot contents" };
        if (dragMode === DragMode.MOVE_FRAME) return { icon: Move, text: modifiers.shift ? "Moving selection (Snapping active)" : "Moving selection (Hold Shift to snap)" };
        if (dragMode === DragMode.RESIZE_FRAME) return { icon: Move, text: "Resizing..." };

        if (isHoveringInteractive) return { icon: MousePointer2, text: "Click to select. Drag to move." };
        ...
        <div className="h-8 bg-panel border-t border-white/10 flex items-center justify-between px-3 select-none z-40 text-[10px] shrink-0">
            ...
            <div className="flex items-center gap-3 text-textMuted flex-1 min-w-0">
                ...
                <div className="flex gap-1 opacity-70 hidden sm:flex">
                    <span className={`px-1.5 py-px rounded border ${modifiers.ctrl ? 'bg-white text-black border-white' : 'bg-transparent border-white/20'}`}>CTRL</span>
                    <span className={`px-1.5 py-px rounded border ${modifiers.shift ? 'bg-white text-black border-white' : 'bg-transparent border-white/20'}`}>SHIFT</span>
                    <span className={`px-1.5 py-px rounded border ${modifiers.alt ? 'bg-white text-black border-white' : 'bg-transparent border-white/20'}`}>ALT</span>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-1 justify-end">
                <div className="flex items-center bg-black/20 rounded-md border border-white/5 overflow-hidden">
                    <button 
                        onClick={() => setViewport({ ...viewport, scale: Math.max(0.05, viewport.scale * 0.8) })} 
