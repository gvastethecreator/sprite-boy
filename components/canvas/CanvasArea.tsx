...
import { useImageLoader, useAssetCache, useRenderLoop, useAutoResetView } from '../../hooks/canvas/useCanvasRenderLoop';
...
const CanvasArea = forwardRef<CanvasHandle, {}>((props, ref) => {
    ...
    const mouse = useCanvasMouse({
        ...
        currentMode, builderCanvas, gridConfig, imageMeta, frames, builderSlots,
        selectedFrameIndex, isEyedropperActive, isSpacePressed,
        onPickColor, onSelectFrame, onUpload, onUpdateSlot, onUpdateSlotEphemeral,
        onUpdateFrame, onUpdateFrameEphemeral, onSwapSlots,
    });

    // Keep refs in sync for the render loop (avoids stale closures in rAF)
    useLayoutEffect(() => {
        propsRef.current = {
            currentMode, imageMeta, gridConfig, builderGrid, frames, selectedFrameIndex, builderCanvas, builderSlots, activeAnimation, playbackFrameIndex, isPlaying, onionSkin, templateConfig, isLoading, loadingMessage, isEyedropperActive, labelConfig,
        };
        stateRef.current = { viewport, dragMode: mouse.dragMode, mousePos: mouse.mousePos, dragSelectionRect: mouse.dragSelectionRect, dragHoverSlot: mouse.dragHoverSlot, isDragOverCanvas: mouse.isDragOverCanvas, dragStartSlot: mouse.dragStartSlot };
    });

    useRenderLoop({ canvasRef, containerRef, propsRef, stateRef, slicerImgObj, assetCache });

    const handleResetView = useAutoResetView(containerRef, builderCanvas, imageMeta, currentMode, activeAnimation ?? null, setViewport);
    ...
    return (
        <main className="h-full bg-app relative flex flex-col select-none group/canvas" onWheel={mouse.handleWheel} onDragOver={mouse.handleDragOver} onDragLeave={() => mouse.setIsDragOverCanvas(false)} onDrop={mouse.handleDrop}>
            {!isEmpty && (
                <CanvasToolbar
                    imageMeta={imageMeta}
            ...
            <div ref={containerRef} className="flex-1 relative overflow-hidden" style={{ cursor: mouse.getCursor() }} onMouseDown={mouse.handleMouseDown} onMouseMove={mouse.handleMouseMove} onMouseUp={mouse.handleMouseUp} onMouseLeave={mouse.handleMouseUp}>
                {isEmpty && (
            ...
            {!isEmpty && <CanvasStatusBar dragMode={mouse.dragMode} mousePos={mouse.mousePos} viewport={viewport} setViewport={setViewport} onResetView={handleResetView} modifiers={modifiers} isHoveringInteractive={selectedFrameIndex !== null} />}
        </main>
    );
});
