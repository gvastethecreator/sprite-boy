...
const Header: React.FC<HeaderProps> = ({ onAnalyzeSheet }) => {
  const {
    ...
    canUndo,
    canRedo,
    handleSaveProject: onSaveProject,
    handleLoadProject: onLoadProject,
    handleNewProject: onNewProject,
    preferences,
      ...
      <button
        ...
        className={`
                ...
                ${
                  isActive
                    ? isBlackAccent
                      ? "text-white bg-zinc-800 shadow-sm border border-white/10"
  ...
  return (
    <header className="h-14 bg-panel border-b border-white/5 flex items-center justify-between px-4 z-50">
      ...
      <input
        type="file"
        ref={projectInputRef}
        onChange={(e) => e.target.files && onLoadProject(e.target.files[0])}
        accept=".json"
        className="hidden"
      />
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <div
            className={`w-8 h-8 bg-surface rounded-lg flex items-center justify-center border border-white/10 group-hover:border-accent/50 transition-colors ${isBlackAccent ? "group-hover:border-zinc-500" : ""}`}
          >
            <LayoutGrid size={18} className={isBlackAccent ? "text-zinc-400" : "text-accent"} />
          </div>
          <div className="flex flex-col justify-center">
            <span className="font-bold text-sm tracking-tight text-textMain leading-none">
              SpriteSlice
            </span>
        ...
        <div className="relative">
          ...
          >
            ...
            <ChevronDown
              size={12}
              className={`opacity-60 transition-transform ${showFileMenu ? "rotate-180" : ""}`}
            />
          </button>
          {showFileMenu && (
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-48 bg-panel border border-border rounded-lg shadow-xl z-50 flex flex-col py-1 animate-scale-in">
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-2 hover:bg-accent/10 hover:text-accent text-left text-xs text-textMain transition-colors"
                >
        ...
        <div className="flex items-center gap-0.5 bg-surface rounded-md border border-white/5 p-0.5">
          <button
            onClick={onUndo}
            disabled={!canUndo}
            className="p-1.5 rounded text-textMuted hover:text-white hover:bg-white/10 disabled:opacity-30 transition-colors"
            title="Undo"
          >
            <Undo2 size={14} />
          </button>
      ...
      <div className="flex items-center gap-3">
          ...
          {showExportMenu && hasImage && (
            <>
              ...
              <div className="absolute top-full right-0 mt-1 w-56 bg-panel border border-border rounded-lg shadow-xl z-50 flex flex-col py-1 animate-scale-in">
                <div className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-textMuted border-b border-white/5">
                  Download As
                </div>
                <button
                  onClick={() => {
                  ...
                  className="flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-left text-xs text-textMain border-b border-white/5 group"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-2 hover:bg-white/5 text-left text-xs text-textMain group"
                >
        ...
        <div className="flex items-center gap-1 pl-3 border-l border-white/10">
          <button
            onClick={onOpenHelp}
            className="p-2 rounded-md text-textMuted hover:text-white hover:bg-white/10 transition-colors"
          >
            <HelpCircle size={16} />
          </button>
          <button
