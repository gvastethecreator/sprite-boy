...
const ALIGNMENTS: { id: SlotAlignment; icon: any }[] = [
  ...
  { id: "top-center", icon: AlignCenterVertical },
  { id: "top-right", icon: AlignEndVertical },
  { id: "middle-left", icon: AlignLeft },
  { id: "center", icon: AlignCenter },
  { id: "middle-right", icon: AlignRight },
  { id: "bottom-left", icon: AlignStartVertical },
  { id: "bottom-center", icon: AlignCenterVertical },
...
const RightSidebar: React.FC = () => {
  const {
    ...
    handleFrameToAsset: onFrameToAsset,
    builderCanvas,
    handleCreateCanvas: onCreateCanvas,
    selectedIndex: selectedSlotIndex,
    builderSlots,
    builderAssets,
    handleAddAsset: onAddAsset,
    handleDeleteAsset: onDeleteAsset,
    handleUpdateSlot: onUpdateSlot,
    animations,
    activeAnimationId,
    handleUpdateAnimation: onUpdateAnimation,
    playbackFrameIndex: selectedKeyframeIndex,
    handleUpdateKeyframe: onUpdateKeyframe,
    handleDeleteKeyframe: onDeleteKeyframe,
    handleDuplicateKeyframe: onDuplicateKeyframe,
    onionSkin,
    setOnionSkin,
  } = useProject();
  ...
  const ResizeHandle = () => (
    ...
    >
      <GripHorizontal
        size={12}
        className="text-textMuted/20 group-hover:text-textMuted transition-colors"
      />
    </div>
  );
    ...
    return (
      <aside className="h-full flex flex-col items-center justify-center p-8 text-center bg-panel border-l border-white/5">
        <div className="w-full animate-slide-in-right">
          ...
          <h3 className="text-sm font-bold text-textMain mb-2 uppercase tracking-widest">
            Init Workspace
          </h3>
          <div className="space-y-4 w-full text-left">
            ...
            <div className="flex items-center gap-2">
              <NumberControl
                value={parseInt(customW)}
                onChange={(v) => setCustomW(v.toString())}
                unit="w"
                className="flex-1"
                labelClassName="hidden"
              />
              <NumberControl
                value={parseInt(customH)}
                onChange={(v) => setCustomH(v.toString())}
                unit="h"
                className="flex-1"
                labelClassName="hidden"
              />
            ...
            <button
              onClick={() => onCreateCanvas?.(parseInt(customW) || 1024, parseInt(customH) || 1024)}
              className="w-full py-2.5 btn-primary text-white rounded-lg text-xs font-bold flex items-center justify-center gap-2 shadow-glow-sm active:scale-95"
            >
      ...
      <aside className="h-full flex flex-col bg-panel border-l border-white/5 overflow-hidden">
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <AnimationProperties
            ...
            keyframeIndex={selectedKeyframeIndex ?? null}
            onUpdateAnim={onUpdateAnimation}
  ...
  if (currentMode === AppMode.BUILDER) {
    ...
    const resetSlot = () => {
      if (selectedSlotData) {
        onUpdateSlot?.(
          selectedSlotData.gridIndex,
          DEFAULT_SLOT_DATA(selectedSlotData.gridIndex, selectedSlotData.assetId),
        );
      }
    };
      ...
      <div className="h-full flex flex-col bg-panel border-l border-white/5 overflow-hidden">
        <SectionHeader
          ...
          action={
            selectedSlotData && (
              <button
                onClick={resetSlot}
                className="p-1 hover:bg-white/10 rounded text-textMuted hover:text-white transition-colors"
                title="Reset Slot"
              >
                <RotateCcw size={14} />
              </button>
            ...
            <div className="p-4 space-y-6 animate-fade-in">
              <div className="flex items-center gap-3 bg-black/30 p-2 rounded-lg border border-white/5">
                <div className="w-10 h-10 bg-checkered rounded overflow-hidden shrink-0 flex items-center justify-center border border-white/10">
                  {builderAssets?.find((a) => a.id === selectedSlotData.assetId) && (
                    <img
                      src={builderAssets.find((a) => a.id === selectedSlotData.assetId)!.src}
                ...
                <div className="min-w-0">
                  <div className="text-[10px] font-bold text-accent uppercase tracking-widest">
                    Cell #{selectedSlotData.gridIndex}
                  </div>
                  <div className="text-[9px] text-textMuted truncate">
                    {builderAssets?.find((a) => a.id === selectedSlotData.assetId)?.name ||
                      "Unknown Asset"}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-bold text-textMuted uppercase tracking-widest flex items-center gap-2">
                  Alignment Point
                </label>
              ...
              <div className="space-y-3 pt-4 border-t border-white/5">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    <NumberControl
                      ...
                      onChange={(v) => updateSlot({ rotation: v })}
                      min={-360}
                      max={360}
                      unit="°"
                      labelClassName="w-8 text-[9px]"
                    />
                  </div>
                  <button
                ...
                <NumberControl
                  icon={Ghost}
                  label="Opacity"
                  value={selectedSlotData.opacity * 100}
                  onChange={(v) => updateSlot({ opacity: v / 100 })}
                  min={0}
                  max={100}
                  slider
                  unit="%"
              ...
              <div className="space-y-3 pt-4 border-t border-white/5">
                <label className="text-[9px] font-bold text-textMuted uppercase tracking-wider block">
                  Geometry
                </label>
                ...
                >
                  <option value="fit">Contain (Fit)</option>
                  <option value="fill">Cover (Fill)</option>
        ...
        <div className="flex items-center gap-1 p-1 bg-white/5 border-b border-white/5 shrink-0">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-[10px] font-bold uppercase transition-all ${activeTab === "layers" ? "bg-accent text-white" : "text-textMuted hover:bg-white/5"}`}
          >
            <Layers size={12} /> Composition
          </button>
          <button
            onClick={() => setActiveTab("library")}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-[10px] font-bold uppercase transition-all ${activeTab === "library" ? "bg-accent text-white" : "text-textMuted hover:bg-white/5"}`}
          >
            <Box size={12} /> Library
          </button>
        </div>
        ...
        <div
          ...
          className="overflow-y-auto custom-scrollbar bg-black/10 shrink-0"
        >
          {activeTab === "layers" && (
            <FrameList
              frames={frames || []}
