...
const BuildTools: React.FC = () => {
    const {
        builderCanvas, slicerImage, currentMode, handleSyncGrid, handleAutoSlice, isLoading,
        isMagicWandActive, setIsMagicWandActive, wandTolerance, setWandTolerance,
        activeGrid, handleSetGridConfig, frames, selectedIndex, handleDuplicateFrame,
        handleFrameToAsset, handleRemoveBackground, handlePreviewBackground, handleCancelPreview,
...
const LeftSidebar: React.FC = () => {
    const { currentMode, animations, activeAnimationId, handleAddAnimation, setActiveAnimationId, handleDeleteAnimation, handleDuplicateAnimation } = useProject();

    if (currentMode === AppMode.TEMPLATE) return <ViewTools />;

    if (currentMode === AppMode.ANIMATION) return (
