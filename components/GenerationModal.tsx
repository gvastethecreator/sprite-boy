...
const GenerationModal: React.FC = () => {
  ...
  const handleGenerate = () => {
    onGenerate(
      ...
      prompt,
      contextType,
      contextType === "library" ? selectedAssetId : prevAsset?.id,
    );
    onClose();
  };
    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <label className="text-xs font-bold text-textMuted uppercase tracking-wider mb-3 block">
              Context Source
            </label>
            <div className="grid grid-cols-1 gap-2">
              ...
              >
                <div className="w-10 h-10 bg-black/20 rounded flex items-center justify-center shrink-0 border border-border/50">
                  {prevAsset ? (
                    <img src={prevAsset.src} className="max-w-full max-h-full p-1" alt="prev" />
                  ) : (
                    <Grid size={16} className="text-textMuted" />
                  )}
                </div>
                <div>
                  <span className="text-sm font-medium text-textMain block">
                    Use Neighboring Frame
                  </span>
                  <span className="text-xs text-textMuted block">
                    {prevAsset ? `Based on "${prevAsset.name}"` : "No previous frame found"}
                  </span>
                </div>
                {contextType === "neighbor" && (
        ...
        <div className="p-4 bg-panel border-t border-border flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 text-xs font-medium text-textMuted hover:text-textMain transition-colors"
          >
            Cancel
          </button>
          <button
