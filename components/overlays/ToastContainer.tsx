...
const ToastItem: React.FC<{ toast: ToastData; onRemove: (id: string) => void }> = ({
  ...
  return (
    ...
    >
      <div
        ref={barRef}
        className={`absolute bottom-0 left-0 h-[2px] bg-current opacity-80 w-full origin-left ${
          toast.type === "error"
            ? "text-red-500"
            : toast.type === "info"
              ? "text-blue-500"
      ...
      <div className="flex items-stretch w-full p-3 gap-3 relative z-10">
        ...
        <div className="flex-1 flex flex-col justify-center min-w-0">
          <span className="text-sm font-medium text-textMain leading-tight truncate">
            {toast.msg}
          </span>
        </div>
