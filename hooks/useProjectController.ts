...
export function useProjectController() {
    ...
    const defaultGrid: GridConfig = { rows: 2, cols: 2, marginX: 0, marginY: 0, paddingX: 0, paddingY: 0 };
    ...
    const [onionSkin, setOnionSkin] = useState<OnionSkinConfig>({ enabled: false, opacity: 0.3, showHitboxes: false });
    ...
    useEffect(() => {
        if (preferences.autoSaveGrid) {
            localStorage.setItem('spriteSlice_ui', JSON.stringify({ mode: currentMode, slicerGrid, builderGrid }));
        }
    ...
    const handleUpload = (file: File) => {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                setProject(prev => ({
                    ...prev,
                    imageMeta: { src, width: img.width, height: img.height, name: file.name, fileSize: file.size },
    ...
    const handleSetAspectRatio = (ratioStr: string) => {
        ...
        let newW, newH;
        if (preset) { newW = preset.w; newH = preset.h; }
        else {
            const [wPart, hPart] = ratioStr.split(':').map(Number);
            const ratio = wPart / hPart;
            const BASE_SIZE = 1024;
            if (ratio >= 1) { newW = BASE_SIZE; newH = Math.round(BASE_SIZE / ratio); }
            else { newH = BASE_SIZE; newW = Math.round(BASE_SIZE * ratio); }
        }
    ...
    const handleSetGridConfig = (c: GridConfig) => {
        const safeC = { ...c, rows: Math.min(256, Math.max(1, c.rows)), cols: Math.min(256, Math.max(1, c.cols)) };
    ...
    const handleGenerateSlot = (slotIndex: number, prompt: string, _contextType: string, contextAssetId?: string) => {
        ...
        builderLogic.runGeneration(prompt, contextImages, slotIndex, setSelectedIndex, ui.genPanel.model, ui.genPanel.mode);
    };
    ...
    const commands = useMemo<CommandPaletteItem[]>(() => [
        ...
        { id: 'mode-build', label: 'Switch to Builder', icon: Box, category: 'View', action: () => handleSetMode(AppMode.BUILDER) },
        { id: 'mode-anim', label: 'Switch to Animation', icon: Layers, category: 'View', action: () => handleSetMode(AppMode.ANIMATION) },
        { id: 'auto-slice', label: 'Auto-Detect Sprites', icon: Wand2, category: 'Tools', action: slicerLogic.handleAutoSlice },
    ...
    return {
        ...
        activeGrid, builderGrid, frames: project.frames, builderSlots: project.builderSlots, 
        animations: project.animations, builderAssets: project.builderAssets,
        ...animLogic, ...slicerLogic, ...builderLogic,
        ...
        handleAddKeyframeFromAsset: (id: string) => { 
            ...
            const slot = (Object.values(project.builderSlots) as SlotData[]).find(s => s.assetId === id); 
            if (slot) animLogic.handleAddKeyframe(slot.gridIndex); 
            else notify("Asset not in grid", "info"); 
        },
        handleSmartFillSlot: (idx: number) => builderLogic.handleSmartFillSlot(idx, builderGrid.cols, setSelectedIndex),
