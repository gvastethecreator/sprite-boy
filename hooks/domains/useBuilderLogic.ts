...
import { ProjectState, SlotData, BuilderCanvasSize, UserPreferences, AIModelId, AIGenerationMode, BuilderAsset } from '../../types';
...
export function useBuilderLogic(
    ...
    const handleDeleteAsset = async (id: string) => {
        ...
        if (assetToDelete) {
            try {
                ...
                setProject(prev => ({ ...prev, builderAssets: prev.builderAssets.filter(a => a.id !== id) }));
    ...
    const handleUpdateSlot = (idx: number, data: SlotData | null) => {
        setProject(prev => {
            ...
            if (data === null) delete slots[idx]; else slots[idx] = data;
    ...
    const handleSwapSlots = (fromIndex: number, toIndex: number) => {
        ...
        setProject(prev => {
            ...
            if (sourceData) slots[toIndex] = { ...sourceData, gridIndex: toIndex }; else delete slots[toIndex];
            if (targetData) slots[fromIndex] = { ...targetData, gridIndex: fromIndex }; else delete slots[fromIndex];
            return { ...prev, builderSlots: slots };
        });
    ...
    const handleFrameToAsset = async (id: number) => {
        ...
        try {
            setIsLoading(true); setLoadingMessage('Cropping asset...');
            const croppedDataUrl = await cropImage(project.imageMeta.src, frame.x, frame.y, frame.w, frame.h);
        ...
        } catch (e) { showToast("Failed to crop frame", "error"); }
        finally { setIsLoading(false); setLoadingMessage(''); }
    };

    const runGeneration = async (prompt: string, contextImages: string[], targetSlotIdx: number | null, setSelectedIndex: (n: number) => void, model: AIModelId, mode: AIGenerationMode) => {
        setIsLoading(true); setLoadingMessage('AI Generating...');
        try {
            ...
            if (preferences.soundEnabled) uiFeedback.play('success');
        } catch (e: any) { showToast("Gen error: " + (e.message || "Unknown"), "error"); }
        finally { setIsLoading(false); setLoadingMessage(''); }
    };

    const handleSmartFillSlot = (slotIndex: number, gridCols: number, setSelectedIndex: (n: number) => void) => {
        const ref = project.builderSlots[slotIndex - 1] || project.builderSlots[slotIndex - gridCols];
        if (ref) { handleUpdateSlot(slotIndex, { ...ref, gridIndex: slotIndex }); setSelectedIndex(slotIndex); showToast("Smart filled", "success"); }
        else showToast("No neighbor to clone", "info");
    };

    return {
        handleAddAsset, handleDeleteAsset, 
        handleUpdateSlot, handleUpdateSlotEphemeral, handleFrameToAsset, runGeneration, 
        handleSmartFillSlot, handleSwapSlots
    };
}
