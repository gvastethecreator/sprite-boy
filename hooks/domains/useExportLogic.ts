...
export function useExportLogic(deps: ExportDeps) {
    ...
    const handleExportZip = useCallback(async (canvasHandle: any) => {
    ...
    const handleExportGif = useCallback(async (animId: string, canvasHandle: any) => {
    ...
    const handleGenerateCode = useCallback((animId: string, scale: number, format: CodeFormat): string => {
        const anim = project.animations.find(a => a.id === animId);
    ...
    }, [project.animations, project.frames, project.imageMeta, project.builderCanvas, currentMode, builderGrid, activeGrid]);
